...
if __name__ == "__main__":
    ...
    if all([os.path.isfile(x) for x in expected_output]):
        print(f"Input genomes found - skipping load", file=sys.stderr)
    else: 
    ...
    if os.path.isfile(antismash_html):
        print(f"Antismash for (input_genomes) exists - skipping...", file=sys.stderr)
    else:
        ...
        print(f"Running Antismash Input Genomes jobid: {runid_antismash_input}", file=sys.stderr)


    ############ Run CAMISIM ############
    if camisim.output_exists(args.readsGB):
        print(f"Camisim output for {args.readsGB}GB found - skipping...", file=sys.stderr)
    else:
        print("Running Camisim", file=sys.stderr)
        ...
        print(f"Running Camisim id {runid_camisim}", file=sys.stderr)
    ...
    ############ Run KMER quantification pipeline ############
    # TODO: obviously....


    ############ Run Assembly pipeline  ############ #TODO: Should we perhaps wrap steps in functions?
    if not args.runAssembly:
        print("Not running assembly pipeline, set --runAssembly flag", file=sys.stderr)
    else:
        ...
        if run_assembly:
            api_Assembler = Assembler(reads_interleaved=reads_interleaved_fp, output_dir=assembly_outdir)
        ...
        if os.path.isfile(antismash_html):
            print(f"Antismash for ({args.readsGB}) exists - skipping...", file=sys.stderr)
        else:
